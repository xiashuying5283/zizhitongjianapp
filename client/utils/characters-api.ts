...
export async function fetchCharacters(): Promise<Character[]> {
  try {
    const response = await fetch(`${EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/characters`);
...
export async function fetchCharactersByTopic(topicId: string): Promise<Character[]> {
  try {
    const response = await fetch(`${EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/characters/by-topic/${topicId}`);
    const result = await response.json();
    if (result.success && result.data) {
      return result.data;
    }
